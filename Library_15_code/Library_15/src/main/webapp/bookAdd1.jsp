...
<body>
	...
	<form action="/Library_15/BookServlet" method="post">
		<table border="1" class=search>
			<tr>
				...
				<td class=searchtd><input type="text" name="isbn" id="max"
					value=${isbn }></td>
				<td class=searchtd><button name="action" value="addsearch"
						class="btn-hover-20">検索</button></td>
			</tr>
		</table>
	</form>
	...
	<c:if test="${exist_true }">
		<form id="form1" action="/Library_15/BookServlet" method="post">
			...
			<div class=center>同じ資料を追加しますか？</div>
			...
			<button class=button-hover-20 type="button" class=button-hover-20
				onclick="popapp1()">登録</button>
		</form>
	</c:if>
	...
	<c:if test="${exist_false }">
		...
		<form id="form2" action="/Library_15/BookServlet" method="post">
			<table border="1" class=result>
				...
				<tr>
					<th id=lable>分類名</th>
					<td class=resulttd><select size="1" id="max1"
						name="category_name">
				...
				<tr>
					<th id=lable>出版社</th>
					<td class=resulttd><input type="text" name="publisher"
						id="max"></td>
				</tr>
				<tr>
					<th id=lable>出版日</th>
					<td class=resulttd><input type="text" id="max"
						name="publish_date" placeholder="yyyymmdd(8桁)"></td>
				</tr>
			</table>
	...
	<script>
		function popapp1() {
			...
			if (result) {
				document.getElementById("actionname").value = "add1";
		...
		function popapp2() {
			...
			if (result) {
				document.getElementById("actionname").value = "add2";
